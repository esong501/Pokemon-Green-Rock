...
// adding a layout for IVs and EVs. Essentially just eliminating the / in the HP display
...
// the following stuff is all for split icons
...
// adding a spriteanim for fairy type
...
static const u8 sMoveTypeToOamPaletteNum[NUMBER_OF_MON_TYPES + CONTEST_CATEGORIES_COUNT] =
{
    ...
    [TYPE_DRAGON] = 15,
    [TYPE_DARK] = 13,
    // adding a palette for Fairy types, but I'm not quite sure why it's 14
    [TYPE_FAIRY] = 14,
...
// code
// code for displaying the split icons
...
{
    // we want to allow the player to view their Pokemon's IVs/EVs from the summary screen
    // they access these stats by pressing L/R/Start, but as a result, we need to 
    // remove the ability to scroll through the pages by pressing LR
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        if (JOY_NEW(DPAD_UP))
        {
        ...
        // three new if else blocks for pulling up different stats
        else if (JOY_NEW(L_BUTTON) && sMonSummaryScreen->currPageIndex == PSS_PAGE_SKILLS)
        {
            BufferIvOrEvStats(0);
        }
...
static bool8 CanReplaceMove(void)
{
    // we want to be able to delete HM moves without some special NPCs so let's do so!
    // if (sMonSummaryScreen->firstMoveIndex == MAX_MON_MOVES
    //     || sMonSummaryScreen->newMove == MOVE_NONE
